...
export const ScreenFilaPlantao: React.FC<ScreenFilaPlantaoProps> = ({
  ...
  // Close floating demo cases menu on click outside or Escape key
  useEffect(() => {
  ...
  return (
    <main className="w-full pt-28 pb-20 bg-[#f8f9ff] min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex flex-col gap-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-3 border-b border-[#e5eeff]">
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 self-start lg:self-center relative">
            {/* Botão Casos de Demonstração com Menu Flutuante (à esquerda do Total Agendados) */}
            <div className="relative" ref={demoMenuRef}>
              <button
                type="button"
                onClick={() => setIsDemoMenuOpen((prev) => !prev)}
                className={`px-3.5 py-2.5 rounded-xl border text-[13px] font-semibold transition-all cursor-pointer shadow-xs flex items-center gap-2 ${
                  isDemoMenuOpen
                    ? 'bg-[#eff6ff] border-[#0284c7] text-[#0284c7] ring-2 ring-[#0284c7]/20 shadow-sm'
                    : 'bg-[#ffffff] hover:bg-[#eff6ff] border-[#d3e4fe] text-[#0369a1]'
                }`}
                title="Abrir cenários clínicos da PoC"
                aria-expanded={isDemoMenuOpen}
                aria-haspopup="true"
              >
                <div className="w-6 h-6 rounded-lg bg-[#eff4ff] text-[#0284c7] flex items-center justify-center shrink-0">
                  <span className="material-symbols-outlined text-[17px]">science</span>
                </div>
                <span>Casos de Demonstração</span>
                <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded-md bg-[#bae6fd]/50 text-[#0369a1]">
                  4
                </span>
                <span
                  className={`material-symbols-outlined text-[16px] text-[#76777d] transition-transform duration-200 ${
                    isDemoMenuOpen ? 'rotate-180 text-[#0284c7]' : ''
                  }`}
                >
                  expand_more
                </span>
              ...
              {/* Menu Flutuante de Casos Sintéticos (estilo dropdown do menu superior) */}
              {isDemoMenuOpen && (
                <div className="absolute right-0 sm:right-auto sm:left-0 top-full mt-2 w-80 sm:w-[460px] md:w-[500px] bg-white rounded-2xl shadow-xl border border-[#d3e4fe] py-3 z-50 animate-fadeIn text-[#0b1c30]">
                  {/* Header do Menu */}
                  <div className="px-4 pb-2.5 border-b border-[#e5eeff] flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold text-[13px] text-[#0b1c30]">Casos Sintéticos de Demonstração</span>
                        <span className="text-[10px] text-[#76777d]">Cenários validados para psiquiatria ambulatorial</span>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setIsDemoMenuOpen(false)}
                      className="w-7 h-7 rounded-lg text-[#76777d] hover:bg-[#f1f5f9] flex items-center justify-center cursor-pointer transition-colors"
                      title="Fechar"
                    >
                  ...
                  {/* Lista de Casos */}
                  <div className="p-3 max-h-[70vh] overflow-y-auto flex flex-col gap-2.5">
                    {/* Caso 1 */}
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="p-3 rounded-xl border border-[#bae6fd] bg-[#f0f9ff] hover:bg-[#e0f2fe] text-left flex flex-col gap-1.5 transition-all cursor-pointer shadow-xs group"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-[13px] text-[#0284c7] group-hover:underline flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full bg-[#0284c7]"></span>
                          Caso 1 (Sucesso & Rascunho Completo)
                        </span>
                        <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-[#bae6fd] text-[#0369a1]">
                          08:30 • Retorno
                        </span>
                      </div>
                      <strong className="text-[12px] text-[#0f172a]">Carla Mendes (29 anos)</strong>
                      ...
                      <div className="pt-1.5 border-t border-[#bae6fd]/60 flex items-center justify-between text-[10px] font-semibold text-[#0284c7]">
                        <span>Abrir Rascunho Completo</span>
                    ...
                    <button
                      ...
                      className="p-3 rounded-xl border border-[#fed7aa] bg-[#fff7ed] hover:bg-[#ffedd5] text-left flex flex-col gap-1.5 transition-all cursor-pointer shadow-xs group"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-[13px] text-[#9a3412] group-hover:underline flex items-center gap-1.5">
                      ...
                      <strong className="text-[12px] text-[#0f172a]">Roberto Alencar (45 anos)</strong>
                      ...
                      <div className="pt-1.5 border-t border-[#fed7aa]/60 flex items-center justify-between text-[10px] font-semibold text-[#c2410c]">
                        <span>Abrir Contradição & Fontes</span>
                        <span className="material-symbols-outlined text-[14px] group-hover:translate-x-0.5 transition-transform">arrow_forward</span>
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      className="p-3 rounded-xl border border-[#fecaca] bg-[#fef2f2] hover:bg-[#fee2e2] text-left flex flex-col gap-1.5 transition-all cursor-pointer shadow-xs group"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-[13px] text-[#991b1b] group-hover:underline flex items-center gap-1.5">
                      ...
                      <strong className="text-[12px] text-[#0f172a]">Lúcia Ferreira (38 anos)</strong>
                      ...
                      <div className="pt-1.5 border-t border-[#fecaca]/60 flex items-center justify-between text-[10px] font-semibold text-[#b91c1c]">
                        <span>Abrir Alerta & Avaliação</span>
                        <span className="material-symbols-outlined text-[14px] group-hover:translate-x-0.5 transition-transform">arrow_forward</span>
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      className="p-3 rounded-xl border border-[#e2e8f0] bg-[#f8fafc] hover:bg-[#f1f5f9] text-left flex flex-col gap-1.5 transition-all cursor-pointer shadow-xs group"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-[13px] text-[#475569] group-hover:underline flex items-center gap-1.5">
                      ...
                      <strong className="text-[12px] text-[#0f172a]">Paulo Silveira (52 anos)</strong>
                      <p className="text-[11px] text-[#475569] leading-relaxed">
                        Paciente recusa consentimento de escuta. Encaminha para documentação médica estritamente manual.
                      </p>
                      <div className="pt-1.5 border-t border-[#cbd5e1] flex items-center justify-between text-[10px] font-semibold text-[#334155]">
                  ...
                  {/* Footer do Menu */}
                  <div className="px-4 pt-2 border-t border-[#e5eeff] flex items-center justify-between text-[10px] text-[#76777d]">
                    <span className="flex items-center gap-1 font-mono">
                      <span className="material-symbols-outlined text-[13px] text-[#0284c7]">verified_user</span>
                      Salvaguardas CFM 2.314/2022
                    </span>
                    <span className="text-[#0284c7] font-semibold">Navegação direta</span>
                  </div>
                </div>
              )}
            </div>
